...
function Body() {
    ...
    const section = ['1' ,'2', '3'];
    const menuItem = ['Trang chủ', 'Giới thiệu', 'Dự án', 'Góc nhìn', 'Liên hệ'];
    const listTitle = {
        0: [UiUx, '-5%', 'UI - UX'],
    ...
    return(
        <>
                    ...
                    {menu ? 
                        (
                            <>
                                ...
                                <div className='fixed left-0 bottom-0 w-full h-full bg-[#082723] flex flex-col justify-center gap-10 capitalize text-white text-6xl font-medium px-24 z-[-1]'>
                                    {menuItem.map((x, index) => (
                                        <div className='w-fit relative transition-all duration-500 hover:text-[#F6AA50]
                                            ...
                                            after:hover:w-full after:hover:border' key={index}>{x}</div>
                                    ))}
                                </div>
                            </>
                        ) : (
            ...
            <div className='flex flex-col overflow-x-hidden'>
                <div className='h-screen flex overflow-y-hidden hidden' id='main'>
                    {/* Section 1 */}
                            ...
                            <div className='w-[630px] flex flex-col gap-8'>
                                ...
                                <div className='flex flex-wrap gap-5'>
                                    {Object.values(listTitle).map((x, index) => ( 
                                        <a onClick={() => {setChangeSection(3); setClickItem(index)}} key={index} className='bg-[#F6AA50] rounded-[50px] text-lg font-semibold uppercase px-10 py-3.5 cursor-pointer'>{x[2]}</a> 
                                    ))}
                                </div>
                            </div>
